import { DemoUser, BUDGET_APPROVAL_LIMITS } from './types';
import { AccountType, StakeholderCategory } from '../../types';

// Helper function to get budget limit
const getBudgetLimit = (accountType: AccountType): number | undefined => {
  const limit = BUDGET_APPROVAL_LIMITS[accountType];
  return limit === null ? undefined : limit;
};

// Extended hierarchical demo users
export const hierarchicalDemoUsers: DemoUser[] = [
  // Level 1: STAFF - Entry-level employees
  {
    id: 'user-1',
    name: '田中太郎',
    department: '営業部',
    role: 'employee',
    position: '営業担当',
    accountType: 'STAFF',
    permissionLevel: 1,
    email: 'tanaka.taro@voicedrive.jp',
    joinDate: new Date('2023-04-01'),
    stakeholderCategory: 'zGen' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=tanaka',
    facility_id: 'kohara_hospital',
    department_id: 'outpatient',
    parent_id: 'user-5',
    budgetApprovalLimit: getBudgetLimit('STAFF'),
    organizationPath: ['user-14', 'user-12', 'user-5', 'user-1']
  },
  {
    id: 'user-2',
    name: '佐藤花子',
    department: '看護部',
    role: 'employee',
    position: '看護師',
    accountType: 'STAFF',
    permissionLevel: 1,
    email: 'sato.hanako@voicedrive.jp',
    joinDate: new Date('2023-06-15'),
    stakeholderCategory: 'frontline' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=sato',
    facility_id: 'kohara_hospital',
    department_id: 'nursing',
    parent_id: 'user-3',
    budgetApprovalLimit: getBudgetLimit('STAFF'),
    organizationPath: ['user-14', 'user-13', 'user-3', 'user-2'],
    expertise: '労働環境改善・法的コンプライアンス'
  },
  
  // Level 2: SUPERVISOR - Senior employees
  {
    id: 'user-3',
    name: '田中太郎',
    department: '看護部',
    role: 'chief',
    position: '看護師長',
    accountType: 'TEAM_LEAD',
    permissionLevel: 2,
    email: 'tanaka.taro@voicedrive.jp',
    joinDate: new Date('2019-04-01'),
    stakeholderCategory: 'frontline' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=tanaka_lead',
    facility_id: 'kohara_hospital',
    department_id: 'nursing',
    parent_id: 'user-6',
    budgetApprovalLimit: getBudgetLimit('TEAM_LEAD'),
    organizationPath: ['user-14', 'user-13', 'user-6', 'user-3'],
    directReports: 12,
    expertise: 'チーム管理・プロジェクトリーダー'
  },
  {
    id: 'user-4',
    name: '鈴木美咲',
    department: 'マーケティング部',
    role: 'employee',
    position: 'シニアマーケター',
    accountType: 'SUPERVISOR',
    permissionLevel: 2,
    email: 'suzuki.misaki@voicedrive.jp',
    joinDate: new Date('2022-03-20'),
    stakeholderCategory: 'frontline' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=suzuki',
    facility_id: 'kohara_hospital',
    department_id: 'other_kohara',
    parent_id: 'user-10',
    budgetApprovalLimit: getBudgetLimit('SUPERVISOR'),
    organizationPath: ['user-14', 'user-10', 'user-4']
  },
  
  // Level 3: DEPARTMENT_HEAD - Department heads
  {
    id: 'user-5',
    name: '山田次郎',
    department: '労務管理部',
    role: 'chief',
    position: '労務管理担当',
    accountType: 'DEPARTMENT_HEAD',
    permissionLevel: 3,
    email: 'yamada.jiro@voicedrive.jp',
    joinDate: new Date('2018-11-01'),
    directReports: 6,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=yamada_jiro',
    facility_id: 'kohara_hospital',
    department_id: 'other_kohara',
    parent_id: 'user-6',
    budgetApprovalLimit: getBudgetLimit('DEPARTMENT_HEAD'),
    organizationPath: ['user-14', 'user-13', 'user-6', 'user-5'],
    expertise: '労務管理・勤務時間制度設計'
  },
  {
    id: 'user-6',
    name: '高橋一郎',
    department: '人財統括本部',
    role: 'manager',
    position: 'マネージャー',
    accountType: 'MANAGER',
    permissionLevel: 3,
    email: 'takahashi.ichiro@voicedrive.jp',
    joinDate: new Date('2017-02-15'),
    directReports: 15,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=takahashi_manager',
    facility_id: 'kohara_hospital',
    department_id: 'other_kohara',
    parent_id: 'user-13',
    children_ids: ['user-3', 'user-5'],
    budgetApprovalLimit: getBudgetLimit('MANAGER'),
    organizationPath: ['user-14', 'user-13', 'user-6'],
    expertise: '人事管理・予算承認・プロジェクト管理'
  },
  
  // Level 4: FACILITY_HEAD - Facility heads
  {
    id: 'user-7',
    name: '渡辺大輔',
    department: 'システム管理部',
    role: 'specialist',
    position: 'システム担当',
    accountType: 'SPECIALIST',
    permissionLevel: 3,
    email: 'watanabe.daisuke@voicedrive.jp',
    joinDate: new Date('2019-07-01'),
    directReports: 4,
    stakeholderCategory: 'technical' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=watanabe',
    facility_id: 'kohara_hospital',
    department_id: 'other_kohara',
    parent_id: 'user-6',
    budgetApprovalLimit: getBudgetLimit('SPECIALIST'),
    organizationPath: ['user-14', 'user-13', 'user-6', 'user-7'],
    expertise: 'システム開発・勤務時間管理システム'
  },
  
  // Level 5: HR_ADMIN - 人財統括本部 戦略企画・統括管理部門（面談予約窓口）
  {
    id: 'user-8',
    name: '中村恵子',
    department: '人財統括本部',
    role: 'hr_admin',
    position: '戦略企画・統括管理部門',
    accountType: 'HR_DEPARTMENT_HEAD',
    permissionLevel: 5,
    email: 'nakamura.keiko@voicedrive.jp',
    joinDate: new Date('2018-04-10'),
    directReports: 8,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=nakamura',
    facility_id: 'kohara_hospital',
    parent_id: 'user-16',
    children_ids: ['user-2'],
    budgetApprovalLimit: getBudgetLimit('HR_DEPARTMENT_HEAD'),
    organizationPath: ['user-14', 'user-16', 'user-8']
  },
  {
    id: 'user-9',
    name: '小林勇気',
    department: '人財統括本部',
    role: 'hr_admin',
    position: '戦略企画・統括管理部門',
    accountType: 'HR_DEPARTMENT_HEAD',
    permissionLevel: 5,
    email: 'kobayashi.yuki@voicedrive.jp',
    joinDate: new Date('2017-09-01'),
    directReports: 6,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=kobayashi',
    facility_id: 'kohara_hospital',
    parent_id: 'user-16',
    children_ids: ['user-3'],
    budgetApprovalLimit: getBudgetLimit('HR_DEPARTMENT_HEAD'),
    organizationPath: ['user-14', 'user-16', 'user-9']
  },
  
  // Level 6: CAREER_SUPPORT_STAFF - 人財統括本部 キャリア支援部門員（面談実施者）
  {
    id: 'user-10',
    name: '加藤真理',
    department: '人財統括本部',
    role: 'career_support_staff',
    position: 'キャリア支援部門員',
    accountType: 'CAREER_SUPPORT_STAFF',
    permissionLevel: 6,
    email: 'kato.mari@voicedrive.jp',
    joinDate: new Date('2016-05-01'),
    directReports: 3,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=kato',
    facility_id: 'kohara_hospital',
    department_id: 'other_kohara',
    parent_id: 'user-13',
    budgetApprovalLimit: getBudgetLimit('CAREER_SUPPORT_STAFF'),
    organizationPath: ['user-14', 'user-13', 'user-10'],
    expertise: 'キャリア支援・HR担当・プロジェクト調整'
  },
  {
    id: 'user-11',
    name: '斎藤孝',
    department: '人財統括本部',
    role: 'career_support_staff',
    position: 'キャリア支援部門員',
    accountType: 'CAREER_SUPPORT_STAFF',
    permissionLevel: 6,
    email: 'saito.takashi@voicedrive.jp',
    joinDate: new Date('2015-10-15'),
    directReports: 6,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=saito',
    facility_id: 'tategami_hospital',
    parent_id: 'user-17',
    budgetApprovalLimit: getBudgetLimit('CAREER_SUPPORT_STAFF'),
    organizationPath: ['user-14', 'user-17', 'user-11']
  },
  
  // Level 7: CAREER_SUPPORT_HEAD - 人財統括本部 各部門長
  {
    id: 'user-16',
    name: '田村智恵',
    department: '人財統括本部',
    role: 'career_support_head',
    position: 'キャリア支援部門長',
    accountType: 'CAREER_SUPPORT_HEAD',
    permissionLevel: 7,
    email: 'tamura.chie@voicedrive.jp',
    joinDate: new Date('2016-03-01'),
    directReports: 15,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=tamura',
    facility_id: 'kohara_hospital',
    parent_id: 'user-17',
    children_ids: ['user-8', 'user-9'],
    budgetApprovalLimit: getBudgetLimit('CAREER_SUPPORT_HEAD'),
    organizationPath: ['user-14', 'user-17', 'user-16']
  },
  {
    id: 'user-17',
    name: '花田美穂',
    department: '人財統括本部',
    role: 'career_support_head',
    position: '戦略企画部門長',
    accountType: 'CAREER_SUPPORT_HEAD',
    permissionLevel: 7,
    email: 'hanada.miho@voicedrive.jp',
    joinDate: new Date('2015-08-01'),
    directReports: 12,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=hanada',
    facility_id: 'tategami_hospital',
    parent_id: 'user-18',
    children_ids: ['user-10', 'user-11', 'user-16'],
    budgetApprovalLimit: getBudgetLimit('CAREER_SUPPORT_HEAD'),
    organizationPath: ['user-14', 'user-18', 'user-17']
  },
  
  // Level 8: HR_DIRECTOR - 人財統括本部 統括管理部門長
  {
    id: 'user-18',
    name: '岡田正樹',
    department: '人財統括本部',
    role: 'hr_director',
    position: '統括管理部門長',
    accountType: 'HR_DIRECTOR',
    permissionLevel: 8,
    email: 'okada.masaki@voicedrive.jp',
    joinDate: new Date('2014-04-01'),
    directReports: 35,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=okada',
    facility_id: 'kohara_hospital',
    parent_id: 'user-14',
    children_ids: ['user-17'],
    budgetApprovalLimit: getBudgetLimit('HR_DIRECTOR'),
    organizationPath: ['user-14', 'user-18']
  },
  
  // Level 9: EXECUTIVE_SECRETARY - 部長・本部長級
  {
    id: 'user-12',
    name: '藤田洋平',
    department: '営業本部',
    role: 'executive',
    position: '営業本部長',
    accountType: 'EXECUTIVE_SECRETARY',
    permissionLevel: 9,
    email: 'fujita.yohei@voicedrive.jp',
    joinDate: new Date('2014-04-01'),
    directReports: 45,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=fujita',
    facility_id: 'kohara_hospital',
    parent_id: 'user-14',
    children_ids: ['user-5', 'user-7'],
    budgetApprovalLimit: getBudgetLimit('EXECUTIVE_SECRETARY'),
    organizationPath: ['user-14', 'user-12']
  },
  {
    id: 'user-13',
    name: '松本由美',
    department: '技術本部',
    role: 'executive',
    position: '技術本部長',
    accountType: 'EXECUTIVE_SECRETARY',
    permissionLevel: 9,
    email: 'matsumoto.yumi@voicedrive.jp',
    joinDate: new Date('2013-07-01'),
    directReports: 60,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=matsumoto',
    facility_id: 'tategami_hospital',
    parent_id: 'user-14',
    budgetApprovalLimit: getBudgetLimit('EXECUTIVE_SECRETARY'),
    organizationPath: ['user-14', 'user-13']
  },
  
  // Level 10: CHAIRMAN - 役員・経営層
  {
    id: 'user-14',
    name: '森田誠',
    department: '経営企画室',
    role: 'executive',
    position: '代表取締役社長',
    accountType: 'CHAIRMAN',
    permissionLevel: 10,
    email: 'morita.makoto@voicedrive.jp',
    joinDate: new Date('2010-01-01'),
    directReports: 8,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=morita',
    facility_id: 'kohara_hospital',
    children_ids: ['user-12', 'user-13', 'user-15', 'user-18'],
    budgetApprovalLimit: getBudgetLimit('CHAIRMAN'),
    organizationPath: ['user-14']
  },
  {
    id: 'user-15',
    name: '橋本明美',
    department: '経営企画室',
    role: 'executive',
    position: '取締役副社長',
    accountType: 'CHAIRMAN',
    permissionLevel: 10,
    email: 'hashimoto.akemi@voicedrive.jp',
    joinDate: new Date('2011-04-01'),
    directReports: 6,
    stakeholderCategory: 'management' as StakeholderCategory,
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=hashimoto',
    facility_id: 'tategami_hospital',
    parent_id: 'user-14',
    budgetApprovalLimit: getBudgetLimit('CHAIRMAN'),
    organizationPath: ['user-14', 'user-15']
  }
];