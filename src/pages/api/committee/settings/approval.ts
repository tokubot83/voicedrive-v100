...
async function handleGet(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.error('承認フロー設定取得エラー:', error);
    return res.status(500).json({ error: '承認フロー設定の取得に失敗しました' });
  }
}
...
async function handlePut(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    AuditService.log({
      userId,
      action: 'COMMITTEE_APPROVAL_SETTINGS_UPDATED',
    ...
    console.error('承認フロー設定更新エラー:', error);
    return res.status(500).json({
      error: '承認フロー設定の更新に失敗しました',
