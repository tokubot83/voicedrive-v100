 ...
 * Phase 2.9: 医療システム連携（2025-10-26）
 *
 * エンドポイント:
 * - POST /api/webhooks/medical-system/whistleblowing/acknowledged     受付確認受信
...
router.post('/acknowledged', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // ペイロード取得
    const { reportId, caseNumber, estimatedResponseTime, acknowledgedAt } = req.body;

    if (!reportId || !caseNumber || !estimatedResponseTime || !acknowledgedAt) {
      return res.status(400).json({
        success: false,
    ...
    await prisma.whistleblowingReport.update({
      ...
      data: {
        ...
        acknowledgementDate: new Date(acknowledgedAt),
        estimatedResponseTime,
        updatedAt: new Date()
      }
    });

    console.log(`[Webhook] 受付確認受信: reportId=${reportId}, caseNumber=${caseNumber}`);
    ...
    console.error('[Webhook] 受付確認Webhook処理エラー:', error);
    return res.status(500).json({
      success: false,
...
router.post('/status-update', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // ペイロード取得
    const { reportId, caseNumber, status, assignedInvestigators, updatedAt, priority } = req.body;
